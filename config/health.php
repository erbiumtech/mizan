...
return [
    ...
    'notifications' => [
        ...
        'mail' => [
            /*
             * Notifications are enabled by default above, so an empty address here means every
             * failed check is reported to nobody — the same silent-success trap as
             * config/backup.php's placeholder recipient, and worth closing the same way.
             * Falls back to the address the application already sends from, so an installation
             * that configures nothing still reaches a real mailbox.
             */
            'to' => env('HEALTH_TO_ADDRESS') ?: env('MAIL_FROM_ADDRESS'),
     ...
     * check has failed.
     *
     * **503, not the package's default of 200.** An uptime monitor polling this endpoint reads
     * the status code, not the body — so 200-on-failure means every external monitor reports the
     * application healthy while the checks inside it are red. That is the failure this whole
     * package exists to prevent, one layer out.
     *
     * Note that nothing currently exposes this endpoint: spatie/laravel-health ships controllers
     * but registers no routes, and none have been added here. A results endpoint reveals disk
     * usage, connection errors and queue state to whoever can reach it, so it wants a
     * deliberate route with `secret_token` or auth in front of it rather than appearing by
     * default. This value is set correctly now so that adding the route later is one line and
     * not one line plus a subtle bug.
     */
