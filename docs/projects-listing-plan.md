...
- **Slack:** `config/services.php` already carries `slack.notifications.bot_user_oauth_token` and a default channel, so `toSlack()` is the natural fit — it needs `composer require laravel/slack-notification-channel`, which isn't installed yet. Until then `via()` returns mail only, driven by `config('projects.alerts.channels')`. Worth noting `MAIL_MAILER` defaults to `log`, so mail needs a real transport configured before any of this is more than a log line. *(Installed 2026-09-26, v3: `'slack'` joined the default channels, each `toSlack()` returns a `SlackMessage`, and `Broadcasting::channels()` drops the channel whenever `SLACK_BOT_USER_OAUTH_TOKEN` is unset — the same one-guard seam that already drops `broadcast` without a socket, so an unconfigured installation queues no slack jobs at all. Alerts post to the default channel via `User::routeNotificationForSlack()`; per-user routing is a deliberate non-feature for now. `tests/Feature/ProjectAlertSlackChannelTest` pins the gate.)*
