...
class ReportDeliveryService
{
    ...
    {
        if (($refusal = $this->memberRefusal($owner)) !== null) {
            return 'The owner '.lcfirst($refusal);
        }
    ...
    private function memberRefusal(User $user): ?string
    {
        ...
        if (! $user->companies()->whereKey($company->getKey())->exists()) {
            return 'No longer a member of this company.';
        }

        return $user->can('ReportView') ? null : 'No longer permitted to read reports.';
    }
